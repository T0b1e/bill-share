<script>
  ...
  function handleFinalizeBy(e) {
    ...
    if ($autoApplyPayer) {
      $transactions = $transactions.map(currentTransaction => {
        if (currentTransaction.id === transaction.id) {
           ...
           const clonedPayers = finalized.map(payer => ({ ...payer, id: uuidv4() }));
    ...
    const isSingleDrop = e.detail.info.trigger === TRIGGERS.DROPPED_INTO_ZONE && finalized.length > 1;
    if (isSingleDrop) {
       const droppedId = e.detail.info.id;
       const droppedUser = finalized.find(user => user.id === droppedId || user.originalUserId === droppedId);
       if (droppedUser) {
  ...
  function openPayerModal(user) {
    ...
    if (transaction.paidBy.length > 1) {
       modalUser = user;
       customAmount = user.customPaidAmount !== undefined ? user.customPaidAmount : "";
       showPayerModal = true;
    }
  }
    ...
    if ($autoApplyPayer) {
      $transactions = $transactions.map(currentTransaction => {
        if (currentTransaction.id === transaction.id) {
          ...
          const clonedPayers = finalized.map(user => ({ ...user, id: uuidv4() }));
          return { ...currentTransaction, paidBy: clonedPayers };
        }
      });
    } else {
  ...
  function removeTransaction() {
    $transactions = $transactions.filter(currentTransaction => currentTransaction.id !== transaction.id);
  }
  ...
  function getAvColor(user) {
     const userId = user.originalUserId || user.id;
     const foundUser = $users.find(u => u.id === userId);
     return foundUser ? foundUser.color : 'bg-stone-200 text-stone-700 border-stone-300';
  }
  ...
  function getPayerLabel(user) {
     ...
     const paidPerPerson = remainingCount > 0 ? Math.max(0, unassignedAmount / remainingCount) : 0;
     
     const userPaid = user.customPaidAmount !== undefined ? user.customPaidAmount : paidPerPerson;
...
{#if showPayerModal && modalUser}
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-[#1c1917]/40 backdrop-blur-sm px-4">
    <div class="bg-[#ffffff] rounded-2xl shadow-xl w-full max-w-sm overflow-hidden p-6 relative">
       <button on:click={() => showPayerModal = false} class="absolute top-4 right-4 text-[#a8a29e] hover:text-[#44403c] cursor-pointer">
         <X size={20} />
       </button>
       <h3 class="text-lg font-medium text-[#292524] mb-2">ยอดเงินที่ {getFullAvName(modalUser)} จ่าย</h3>
       <p class="text-sm text-[#78716c] mb-4 tracking-wide">ปล่อยว่างเพื่อหารเท่าๆ กัน (Leave blank for equal split)</p>
       
       <div class="flex items-center gap-2 mb-6">
         <input type="number" bind:value={customAmount} min="0" class="flex-1 bg-[#fafaf9] border border-[#e7e5e4] text-[#292524] rounded-lg px-4 py-2 outline-none focus:border-[#78716c]" placeholder="จำนวนเงินดิบ (บาท)" />
         <span class="text-[#78716c] font-medium">บาท</span>
       </div>

       <div class="flex gap-3">
         <button on:click={() => { customAmount = ""; savePayerAmount(); }} class="flex-1 bg-[#f5f5f4] hover:bg-[#e7e5e4] text-[#44403c] py-2 rounded-lg font-medium cursor-pointer transition">หารเท่ากัน</button>
         <button on:click={savePayerAmount} class="flex-1 bg-[#292524] hover:bg-[#1c1917] text-[#ffffff] py-2 rounded-lg font-medium cursor-pointer transition shadow-sm active:scale-95">บันทึก</button>
       </div>
    </div>
  </div>
{/if}

<div class="flex flex-col lg:flex-row gap-6 py-2 border-b border-stone-100 last:border-0 relative group items-start">
  <!-- Left info -->
  <div class="flex-grow flex flex-col gap-3 w-full lg:w-auto">
    <div class="flex flex-wrap items-center justify-between gap-3 w-full">
      ...
      <div class="flex items-center gap-1.5 shrink-0 px-3 py-1.5 rounded-lg">
        <span class="text-stone-700 font-medium text-sm">{(transaction.qty * transaction.price).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
  ...
  <div class="flex flex-col sm:flex-row gap-4 w-full lg:w-[420px] shrink-0">
    <div class="flex flex-col gap-1 flex-1">
      <div class="px-1 text-xs text-stone-400 font-medium tracking-wide mt-1">จ่ายโดย</div>
      <div class="min-h-[48px] bg-stone-50 border border-dashed border-stone-200 rounded-xl p-1.5 flex flex-wrap gap-1.5 items-center transition-colors shadow-inner"
        ...
        {#each paidByItems as user (user.id)}
          ...
          >
            ...
            {#if paidByItems.length > 1}
               <div class="absolute -bottom-1.5 -right-2 bg-stone-800 dark:bg-stone-600 text-white text-[9px] px-1 rounded-sm shadow-sm leading-none py-0.5 pointer-events-none z-10 font-bold tracking-tight">
                 {getPayerLabel(user)}
               </div>
            {/if}
          </button>
        ...
        {#if paidByItems.length === 0}
          <div class="text-xs text-stone-400 w-full text-center py-2 pointer-events-none select-none">ลากเพื่อนมาวาง</div>
        {/if}
      </div>
    </div>
    ...
    <div class="flex flex-col gap-1 flex-1">
      <div class="flex justify-between items-center px-1">
        <div class="text-xs text-stone-400 font-medium tracking-wide mt-1">ใช้โดย</div>
        <button on:click={shareWithAll} class="text-[10px] text-stone-500 font-semibold uppercase bg-stone-100 hover:bg-stone-200 transition-colors px-2 py-0.5 rounded cursor-pointer shadow-sm active:shadow-none active:translate-y-[1px]">ทุกคน</button>
      </div>
      <div class="min-h-[48px] bg-stone-50 border border-dashed border-stone-200 rounded-xl p-1.5 flex flex-wrap gap-1.5 items-center transition-colors shadow-inner"
  ...
  {#if $transactions.length > 1}
    <button on:click={removeTransaction} class="absolute top-4 right-1 lg:-right-4 text-stone-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition p-1 bg-white rounded-full">
      <X size={18} />
    </button>
  {/if}
</div>
